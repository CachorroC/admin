import { Demanda, Deudor, Grupo, IntCarpeta, TipoProceso } from '#@/lib/types/demandados';
import { getProceso } from '#@/lib/RamaJudicial';

export const ramaApiEndpoint = () => {
...
export class Despacho  {
  ...
  constructor (
    {
      id = 0, tipo = 'sin tipo', url = 'https://www.ramajudicial.gov.co'
    }:{id: number, tipo: string, url?: string}
  ) {
    this._id = id;
    this._tipo = tipo;
    this._url = `${ ramaApiEndpoint() }/${ url }`;
  }
  get id() {
    return this._id;
  }
  ...
  set url(
    ...
    this._url = `${ ramaApiEndpoint() }/${ despacho }`;
  }
}
...
for ( const carpeta of carpetas ) {
  ...
  if ( demanda ){
    const despachoOrigen = new Despacho(
      demanda.juzgado[ 0 ]
    );
    ...
    const filterProcesos = procesos.find(
      (
        proceso
      ) => {
        return proceso.idProceso === carpeta.idProceso;
      }
    );

    if ( filterProcesos ) {
      despachoOrigen.url(filterProcesos.despacho.replace(
        ' ', '-'
      ););
    }
  }
