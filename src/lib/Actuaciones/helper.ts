...
import { carpetasCollection, getCarpetas } from '#@/lib/Carpetas';
import { MonCarpeta, carpetaConvert } from '#@/lib/types/carpeta';
...
export async function idkyet () {
  ...
  for ( const carpeta of carpetas ) {
    ...
    if ( actuaciones ) {
      ...
      const updtCarp = await collection.findOneAndUpdate(
        {
          _id: new ObjectId(
            carpeta._id
          )
      ...
      if ( updtCarp.ok ) {
        ...
        if ( nCarp ) {
          const newnewCarp = carpetaConvert.toMonCarpeta(
            nCarp
          );
          Carpetas.set(
            newnewCarp._id, newnewCarp
          );
        }
      }
    }
