    ...
    const juzgados = await Promise.all(
      procesos.map(
        async (
          ...
          return <JuzgadosByllaveProceso key={proceso._id} llaveProceso={ proceso.llaveProceso} />;
        }
      )
    );
    return <>{juzgados}</>;
...
export async function getActuacionesByidProceso (
  { idProcesos }: { idProcesos: number[] | undefined; }
) {
...
export async function fetchFecha (
  { proceso }: { proceso: monCarpetaDemandado }
) {
  const acts = (await getActuacionesByidProceso(
    { idProcesos: proceso.idProceso }
  )).flat();
